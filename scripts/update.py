...
for a in soup.find_all("a"):
    if "discussion guide" in a.get_text(strip=True).lower():
        ...
        # Strategy 1: Prefer the date encoded in the PDF filename — it's
        # unambiguous and per-link. Ancestor-text scraping can pick up dates
        # from other guides that share a common container.
        url_date = _extract_date_from_url(full_url, year_hint=today.year)
        if url_date:
            guides.append((url_date, full_url))
            continue

        # Strategy 2: Fall back to nearby-text dates (limited to the link
        # and its immediate parent so we don't pull dates from siblings).
        local_ctx = a.get_text(" ", strip=True)
        if a.parent:
            local_ctx += "  " + a.parent.get_text(" ", strip=True)
        ds = _extract_dates_from_text(local_ctx, year_hint=today.year)
...
for a in soup_r.find_all("a"):
    if "discussion guide" in a.get_text(strip=True).lower():
        ...
        # Prefer the date encoded in the PDF filename (unambiguous, per-link).
        # On a series-resources page every link shares the same ancestor
        # container, so ancestor-text date extraction returns ALL dates on
        # the page and assigns every link the same "latest" date.
        url_date = _extract_date_from_url(full_url, year_hint=today.year)
        if url_date:
            dt_choice = url_date
        else:
            # Fall back to nearby-text dates, but only consider the immediate
            # link/parent text — not deep ancestors that span the whole list.
            local_ctx = a.get_text(" ", strip=True)
            if a.parent:
                local_ctx += "  " + a.parent.get_text(" ", strip=True)
            ds = _extract_dates_from_text(local_ctx, year_hint=today.year)
...
**import**(“json”).dumps(payload, indent=2, ensure_ascii=False),
encoding=“utf-8”
)
